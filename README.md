...
* **LV1：環境開發階段**
    * **目標：** 專注於環境架設。包含申請氣象署 API 金鑰、Fork 前後端程式碼，並成功部署至 Zeabur 與 GitHub Pages，確保前後端數據連線正常運作。
* **LV2：個人風格改造 (Vibe Coding)**
    * **目標：** 專注於前端風格改造。透過調整配色、字體、文案與圖示，將基礎版 App 轉變為具備個人化風格的「阿強動態氣象站」。

---

## 🛠️ 使用技術與工具 (Technologies Used)
...
## ✨ 網站頁面結構 (Page Structure)

本應用程式採用 Mobile-First 流動佈局，主要結構為垂直堆疊的區塊：

1.  **頂部狀態列 (`.status-bar`)**
    * 功能：顯示**縣市選擇下拉選單**、**當前日期**及**時間**。
    * 特色：兩個主要標籤皆為深色膠囊造型，保證白色文字對比度。
2.  **今日英雄卡 (`#heroCard`)**
    * 功能：顯示當前主要天氣狀態、溫度、和兩項快速建議（雨具/穿搭）。
